...
public class ModEvents {
    ...
    public void disablePlantingFarmLand(PlayerInteractEvent.RightClickBlock event){
        if (event.getEntity().getMainHandItem().getItem().equals(ModItems.WARPED_WART_SEED.get())) {
            if (event.getEntity().level().getBlockState(event.getPos()).getBlock().equals(Blocks.FARMLAND) ||
                    event.getEntity().level().getBlockState(event.getPos()).getBlock().equals(Blocks.SOUL_SOIL)) {
    ...
    public void onHoeEventSoulSoil(PlayerInteractEvent.RightClickBlock event) {
        if (event.getEntity().getMainHandItem().getItem() == ModItems.WARPED_WART_SEED.get()) {
            if (event.getEntity().level().getBlockState(event.getPos()).getBlock().equals(Blocks.SOUL_SOIL)) {
                event.getEntity().level().setBlockAndUpdate(event.getPos(), ModBlocks.WARPED_SOUL_SOIL.get().defaultBlockState());
